...
def enrich_with_yf(tickers: list[str]) -> pd.DataFrame:
    ...
    for i, t in enumerate(to_fetch, 1):
        ...
        if i % 120 == 0:
            print(f"[YF] {i}/{len(to_fetch)} enriched…"); time.sleep(0.8)
...
def try_fill_sector_with_yfinfo(df: pd.DataFrame, ticker_col="ticker_yf") -> pd.DataFrame:
    ...
    df["sector_fill"] = sec; df["industry_fill"] = ind
    return df

def try_fill_sector_with_av(df: pd.DataFrame, ticker_col="ticker_yf") -> pd.DataFrame:
    if not AV_KEY or df.empty:
        df["av_sector"] = None; df["av_industry"] = None; return df
    sec, ind = [], []; done = 0
    for i, sym in enumerate(df[ticker_col].tolist(), 1):
        if done >= MAX_AV_LOOKUPS: sec.append(None); ind.append(None); continue
        try:
            url = "https://www.alphavantage.co/query"; params = {"function":"OVERVIEW","symbol":sym,"apikey":AV_KEY}
            r = requests.get(url, params=params, timeout=30)
            if r.status_code == 200:
                j = r.json(); sec.append(j.get("Sector")); ind.append(j.get("Industry")); done += 1
            else: sec.append(None); ind.append(None)
        except Exception: sec.append(None); ind.append(None)
        time.sleep(AV_SLEEP)
